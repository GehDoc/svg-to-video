...
async function main() {
  ...
  program
    ...
    * @param {number} fps
    * @param {string} outDir
    * @param {{ keepFrames: boolean; hold: number; force: boolean; resolution: string; scale: number }} options
    */
    ...
    console.log(
    `  Settings:   ${duration}s @ ${fps}fps (Hold: ${options.hold}s, Resolution: ${options.resolution}, Scale: ${options.scale}x)`
    );
    ...
    await createFrames(svg, fps, totalFrames, padWidth, outDir, puppeteerArgs, options.resolution, options.scale);
