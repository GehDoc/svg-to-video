import{a as e,n as t}from"./chunk-DnJy8xQt.js";import{a as n}from"./iframe-CtbJlkO-.js";import{t as r}from"./jsx-runtime-DxP0NviS.js";function i(e){document.getAnimations().forEach(t=>{try{t.pause()}catch{}t.currentTime=e}),document.querySelectorAll(`svg`).forEach(t=>{try{typeof t.pauseAnimations==`function`&&t.pauseAnimations(),typeof t.setCurrentTime==`function`&&t.setCurrentTime(e/1e3)}catch{}})}var a=t((()=>{(function(){typeof module<`u`&&module.exports&&(module.exports={seekAnimations:i})})()}));function o(e,t){let n=`fill.fill-opacity.fill-rule.stroke.stroke-opacity.stroke-width.stroke-linecap.stroke-linejoin.stroke-miterlimit.stroke-dasharray.stroke-dashoffset.opacity.display.visibility.filter.mask.clip-path.clip-rule.stop-color.stop-opacity.font-family.font-size.font-weight.font-style.text-anchor.text-decoration.dominant-baseline.alignment-baseline.baseline-shift.transform.transform-origin.x.y.width.height.cx.cy.r.rx.ry.color.flood-color.flood-opacity.lighting-color.mix-blend-mode.isolation`.split(`.`),r=document.getElementById(`svg-container`),i=document.getElementById(`capture-canvas`),a=!1;window.addEventListener(`message`,async o=>{let{type:s,payload:c}=o.data;if(s===`LOAD_SVG`){a=!1;let{svgContent:n,width:o,height:s,timeMs:l}=c;r.innerHTML=n,r.style.width=o+`px`,r.style.height=s+`px`,r.style.backgroundColor=`transparent`,i.width=o,i.height=s,e(l),requestAnimationFrame(()=>{a=!0,window.parent.postMessage({type:`READY`},t)})}if(s===`SEEK`){if(!a)return;e(c.timeMs),await new Promise(e=>requestAnimationFrame(e)),window.parent.postMessage({type:`SEEKED`},t)}if(s===`CAPTURE`){if(!a)return;let e=r.querySelector(`svg`),o=i.getContext(`2d`);if(!e||!o)return;let s=e.cloneNode(!0);s.querySelectorAll(`animate, animateTransform, animateMotion, set`).forEach(e=>e.remove());let l=[e,...Array.from(e.querySelectorAll(`*`)).filter(e=>![`animate`,`animateTransform`,`animateMotion`,`set`].includes(e.tagName.toLowerCase()))],u=[s,...Array.from(s.querySelectorAll(`*`))];l.forEach((e,t)=>{let r=window.getComputedStyle(e),i=u[t];if(!(!i||!i.style))if(c.method===`high-fidelity`)for(let e=0;e<r.length;e++){let t=r[e];i.style.setProperty(t,r.getPropertyValue(t),r.getPropertyPriority(t))}else for(let e of n){let t=r.getPropertyValue(e);t&&i.style.setProperty(e,t)}});let d=new XMLSerializer().serializeToString(s),f=new Blob([d],{type:`image/svg+xml;charset=utf-8`}),p=URL.createObjectURL(f),m=new Image;await new Promise((e,t)=>{m.onload=e,m.onerror=t,m.src=p}),o.clearRect(0,0,i.width,i.height),o.drawImage(m,0,0,i.width,i.height),URL.revokeObjectURL(p);let h=await createImageBitmap(i);window.parent.postMessage({type:`CAPTURE_RESULT`,payload:h},t,[h])}}),window.parent.postMessage({type:`SCRIPT_LOADED`},t)}var s=t((()=>{})),c,l=t((()=>{c=`<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
